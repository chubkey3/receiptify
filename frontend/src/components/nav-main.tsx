      ...
      <SidebarGroupContent className="flex flex-col gap-2">
        <SidebarMenu>
          <SidebarMenuItem className="flex items-center gap-2">
            ...
            <UploadDrawer analyticsMutate={analyticsMutate} expensesMutate={expensesMutate}/>
            {/*<Button
              size="icon"
...
function UploadDrawer({analyticsMutate, expensesMutate}: {analyticsMutate: KeyedMutator<{value: Summary}>, expensesMutate: KeyedMutator<Expense[]>}) {
  ...
  const handleUpload = async () => {
    ...
    try {            
      ...
      toast.error("Image upload failed!", {position: "top-center", description: `Status Code: ${response?.status} | Error: ${error}`});
    }
    toggleLoading(false);
    if (response && response.status === 200) {    
      ...
      toast.success("Receipt uploaded successfully!", {position: "top-center"})
    } else {
      toast.error("Image upload failed!", {position: "top-center", description: `Status Code: ${response?.status} | Error: ${response?.statusText}`});
    }    
  };  
  ...
  return (
    <Drawer direction={"bottom"}>
      ...
      <DrawerContent>
            ...
            <>              
              ...
              <div className="grid gap-2">
                ...
                <div className="text-muted-foreground">
                  Make sure your receipt is taken in good lighting conditions where all text is visible. 
                </div>
              </div>
              <Separator />
            </>
        ...
        <DrawerFooter>
          <Button onClick={handleUpload}>{isLoading ? <div className={`h-5 w-5 animate-spin rounded-full border-2 border-muted-foreground border-t-transparent`} /> : "Upload"}</Button>
          <DrawerClose asChild>
